...
export function SiteHeader(){const pathname=usePathname();const home=pathname==='/';return <><a className="skip-link" href="#main-content">Skip to content</a><header className={`site-header centered-brand ${home?'home-header':''}`}><a className="wordmark" href="/">AK<span className="brand-star" aria-hidden="true">✦</span></a>{!home&&<SiteNav/>}</header>{!home&&<RotatingEarth width={700} height={700} className="internal-globe-background"/>}</>}
