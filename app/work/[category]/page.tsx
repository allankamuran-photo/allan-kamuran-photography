...
export default async function GalleryPage({params}:{params:Promise<{category:string}>}){const {category}=await params;if(category==='nature')redirect('/work/travel');const current=categories.find(c=>c.slug===category);if(!current)notFound();return <CategoryGallery category={current} photos={categoryPhotos(current.slug)}/>}
